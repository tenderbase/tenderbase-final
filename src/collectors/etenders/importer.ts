...
export async function persistRelease(release: Release) {
  ...
  for (const d of t.documents ?? []) {
    const documentId = d.id ?? d.supportDocumentId ?? d.blobName ?? hash(d).slice(0, 24);
    const existing = await db.document.findUnique({ where: { tenderId_documentId: { tenderId: tender.id, documentId } } });
    const base = {
      documentType: d.documentType, title: d.title, description: d.description, format: d.format, url: d.url,
      datePublished: asDate(d.datePublished), dateModified: asDate(d.dateModified), supportDocumentId: d.supportDocumentId,
      blobName: d.blobName, downloadedFileName: d.downloadedFileName,
    };
    if (existing) {
      await db.document.update({ where: { id: existing.id }, data: { ...base, downloadStatus: existing.downloadStatus === 'downloaded' ? 'downloaded' : (d.blobName ? 'discovered' : 'missing_blob_name') } });
    } else {
      await db.document.create({ data: { tenderId: tender.id, documentId, ...base, downloadStatus: d.blobName ? 'discovered' : 'missing_blob_name' } });
    }
  }
